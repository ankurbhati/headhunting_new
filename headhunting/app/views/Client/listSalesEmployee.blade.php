                            ...
                            <td>{{($user->created_at != "" && $user->created_at != "0000-00-00 00:00:00")?date("Y-m-d", strtotime($user->created_at)):"-"}}</td>
		                        <td>
		                        	@if(Auth::user()->getRole() == 1 || Auth::user()->hasRole(8))
		                        		<a href="{{ URL::route('client-transfer', array('id' => $clientId, 'userId' => $user->id)) }}" title="Change Password" class="btn btn-primary btn-white">Tranfer To {{$user->first_name. " ".$user->last_name }}</a>
		                        	@endif
		                        </td>
		                      </tr>
      						@endforeach
